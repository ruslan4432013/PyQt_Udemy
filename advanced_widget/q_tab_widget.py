...
class DlgMain(QDialog):
    def __init__(self):
        ...
        self.tab_main.setTabPosition(QTabWidget.South)
        self.tab_main.setMovable(True)
        self.tab_main.setTabsClosable(True)
        self.tab_main.tabCloseRequested.connect(self.evt_tab_close)
    ...
    def setup_layout(self):
        ...
        self.lyt_main.addWidget(self.tab_main)

        # Add Widgets To Left Layout
        # self.lyt_left.addWidget(self.cmb_selector)
        # self.lyt_left.addStretch()

        # Add Tabs To tab_widget
        self.tab_main.addTab(self.wdg_general, 'General')
        self.tab_main.addTab(self.wdg_species, 'Species')
        self.tab_main.addTab(self.wdg_location, 'Location')
        self.tab_main.addTab(self.wdg_surveys, 'Surveys')
    ...
    def evt_tab_close(self, idx):
        ans = QMessageBox.question(self,
                                   'Close',
                                   f'Do you really want to remove the "{self.tab_main.tabText(idx)}" tab?')
        if ans == QMessageBox.Yes:
            self.tab_main.removeTab(idx)
